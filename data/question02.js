...
var demo07 = {
    ...
    data: [{
        ...
        data: [
            { context: "InnoDB也使用B+Tree作为索引结构" },
            { context: "而在InnoDB中，表数据文件本身就是按B+Tree组织的一个索引结构，这棵树的叶节点data域保存了完整的数据记录。这个索引的key是数据表的主键，因此InnoDB表数据文件本身就是主索引" },
            { context: "InnoDB的辅助索引data域存储相应记录主键的值而不是地址。换句话说，InnoDB的所有辅助索引都引用主键作为data域。" },
            { context: "适合索引的字段" },
            { context: "1. 表的主键、外键必须有索引" },
            { context: "2. 数据量超过300的表应该有索引" },
            { context: "3. 经常与其他表进行连接的表，在连接字段上应该建立索引" },
            { context: "4. 经常出现在where字句中的字段，特别是大表的字段，应该建立索引" },
            { context: "5. 索引应该建在选择型高的字段上" },
            { context: "6. 索引应该建在小字段上，对于大的文本字段甚至超长字段，不要建索引" },
            { context: "7. 复合索引的建立需要进行仔细分析，尽量考虑使用单字段索引代替：" },
            { context: "  ① 正确选择复合索引中的主列字段，一般是选择性较好的字段" },
            { context: "  ② 复合索引的杰哥字段是否经常同时以AND方式出现在where子句中？单字段查询是否极少甚至没有？如果是，则可以建立复合索引；否则考虑单字段索引；" },
            { context: "  ③ 如果复合索引中包含的字段经常单独出现在Where子句中，则分解为多个单字段索引；" },
            { context: "  ④ 如果复合索引所包含的字段超过3个，那么仔细考虑其必要性，考虑减少复合的字段；" },
            { context: "  ⑤ 如果既有单字段索引，又有这几个字段上的复合索引，一般可以删除复合索引；" },
            { context: "8. 频繁进行数据操作的表，不要建立太多的索引；" },

        ]
    }, {
        title: "2、垃圾回收期和垃圾回收机制简单说一下",
        data: [
            { context: "可达性分析算法程序把所有的引用关系看作一张图，从一个节点GC ROOT开始，寻找对应的引用节点，找到这个节点以后，继续寻找这个节点的引用节点，当所有的引用节点寻找完毕之后，剩余的节点则被认为是没有被引用到的节点，即无用的节点，无用的节点将会被判定为是可回收的对象。" },
            { context: "这个算法的基本思路就是通过一系列的称为“GC Roots”的对象作为起始点，从这些节点开始向下搜索，搜索所走过的路径称为引用链（Reference Chain），当一个对象到GC Roots没有任何引用链相连（用图论的话来说，就是从GC Roots到这个对象不可达）时，则证明此对象是不可用的。" },
            { context: "无用的类则需要同时满足下面3个条件 垃圾回收机制   其他可以用可达性算法" },
            { context: "该类所有的实例都已经被回收，也就是Java堆中不存在该类的任何实例；" },
            { context: "加载该类的ClassLoader已经被回收；" },
            { context: "该类对应的java.lang.Class对象没有在任何地方被引用，无法在任何地方通过反射访问该类的方法。" },
            { context: "" },
            { context: "废弃常量和无用的类" },
            { context: "" },
            { context: "" },
        ...
        title: "3、OOM都见过哪些  Out Of Memory  内存溢出",
        data: [
            { context: "<a href=\"https://blog.csdn.net/S_Alics/article/details/103306027\">内存溢出</a>" },
        ]
    }, {
        title: "4、虚拟机参数调整过什么",
        data: [
            { context: "" },
            { context: "<a href=\"https://blog.csdn.net/BaiLingChen03/article/details/81588427\">虚拟机参数</a>" },
            { context: "" },
            { context: "" },
            { context: "" },
        ]
